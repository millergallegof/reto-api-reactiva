...
public class ResourceRouter {
    ...
    @Bean
    public RouterFunction<ServerResponse> addUnitsAvailable(AddUnitsAvailableUseCase addUnitsAvailableUseCase) {
        return route(
                PUT("/add/{id}/{units}"),
                request -> ServerResponse.ok()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(BodyInserters
                                .fromPublisher(addUnitsAvailableUseCase
                                                .apply(request.pathVariable("id"), Integer.valueOf(request.pathVariable("units"))),
                                        ResourceDTO.class))
        );
    }
